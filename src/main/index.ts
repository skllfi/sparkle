...
ipcMain.handle("tray:set", (event, value: boolean) => {
...
ipcMain.handle("nodpi:update-blacklist", (event, content: string) =>
...
ipcMain.handle(
  "proxy:set",
  (event, { address, port }: { address: string; port: number }) =>
...
app.whenReady().then(async () => {
  ...
  if (store.get("showTray")) {
    setTimeout(() => {
      if (mainWindow) trayInstance = createTray(mainWindow);
    }, 50);
  }
  setTimeout(() => {
  ...
  app.on("quit", async () => {
    const nodpiStatus = await checkNoDPIStatus();
