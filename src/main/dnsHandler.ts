...
export const setupDNSHandlers = (): void => {
  ipcMain.handle("dns:get-current", async () => {
    try {
      ...
      if (result.success) {
        ...
        const dnsInfo: DnsInfo[] = lines.map((line: string) => {
  ...
  ipcMain.handle(
    ...
    async (
      event,
      {
        ...
        primaryDNS = "",
        secondaryDNS = "",
      }: { dnsType: string; primaryDNS?: string; secondaryDNS?: string },
    ) => {
      try {
  ...
  ipcMain.handle(
    "dns:test",
    async (event, { hostname = "google.com" }: { hostname?: string }) => {
      try {
        const script = `
        try {
    ...
    try {
      ...
      if (result.success) {
        const lines = result.output
          ...
          .split("\n")
          .filter((line: string) => line.includes("|"));
        const adapters: AdapterInfo[] = lines.map((line: string) => {
