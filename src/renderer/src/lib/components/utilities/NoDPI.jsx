    ...
    <>
        <Card className="mb-4">
            <div className="flex flex-col h-full">
                <h3 className="text-lg font-semibold">{t("utilities.nodpi_title")}</h3>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <div className="text-sm text-gray-400">{t("utilities.nodpi_status_label")}</div>
                        <div className={`font-semibold ${status === 'active' ? 'text-green-400' : 'text-red-400'}`}>
                            {status === 'active' ? t("utilities.nodpi_status_active") : t("utilities.nodpi_status_inactive")}
                        </div>
                    </div>
                    <div className="flex items-center space-x-2">
                        <Toggle checked={autostart} onCheckedChange={toggleAutostart} />
                        <span className="text-sm">{t("utilities.nodpi_autostart_label")}</span>
                    </div>
                </div>

                {status === 'active' && (
                <div className="bg-gray-800 p-3 rounded-lg mb-4 text-xs">
                    <h4 className="font-semibold mb-2">{t("utilities.nodpi_stats_title")}</h4>
                    <div className="grid grid-cols-3 gap-2">
                        <div><strong>{t("utilities.nodpi_stats_conns")}:</strong> {stats.conns}</div>
                        <div><strong>{t("utilities.nodpi_stats_unblock")}:</strong> {stats.unblock}</div>
                        <div><strong>{t("utilities.nodpi_stats_errors")}:</strong> {stats.errors}</div>
                        <div><strong>{t("utilities.nodpi_stats_traffic")}:</strong> {stats.traffic}</div>
                        <div className="col-span-2"><strong>{t("utilities.nodpi_stats_speed")} (DL/UL):</strong> {stats.speed_dl} / {stats.speed_ul}</div>
                    </div>
                </div>
                )}
                ...
                <Button onClick={toggleNoDPI} className="flex-1">
                    {status === 'inactive' ? t("utilities.nodpi_run") : t("utilities.nodpi_stop")}
                </Button>
        ...
        {showModal && (
            <Modal open={showModal} onClose={() => setShowModal(false)}>
                <div className="bg-sparkle-card p-6 rounded-2xl border border-sparkle-border text-sparkle-text w-[90vw] max-w-lg">
                    <h2 className="text-lg font-semibold mb-4">{t("utilities.nodpi_edit_domains_title")}</h2>
                    <p className="text-sm text-gray-400 mb-4">
