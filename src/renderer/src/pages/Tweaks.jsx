...
function Tweaks() {
  ...
  const categories = useMemo(
    () => [
      "All",
      ...new Set(tweaks.flatMap((t) => t.category || []).filter(Boolean)),
    ],
    [tweaks],
  );
    ...
    return tweaks.filter((tweak) => {
      ...
      const matchesCategory =
        ...
        (Array.isArray(tweak.category) &&
          tweak.category.includes(activeCategory)) ||
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button
              onClick={async () => {
                ...
                const loadingToastId = toast.loading(
                  `Applying tweak: ${selectedTweak.title}`,
                );
                ...
                try {
                  ...
                  console.error(
                    `Error applying tweak ${selectedTweak.title}:`,
                    error,
                  );
                  log.error(
                    `Error applying tweak ${selectedTweak.title}:`,
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
            {sortedTweaks.length > 0 ? (
                  ...
                  <Card key={originalIndex} className=" p-0 h-52">
                    <div className="p-5 flex flex-col h-[260px]">
                          ...
                          {(() => {
                            ...
                            return (
                              <>
                                ...
                                {tweak.reversible == null ||
                                tweak.reversible == true ? (
                                  <Tooltip
                                    content={
                                      !compatibility.compatible
                                  ...
                                  >
                                    <Toggle
                                      checked={
                                        toggleStates[tweak.name] || false
                                      }
                                      onChange={(e) => {
                                        const willOpenModal =
                                          !!tweak.modal &&
                                          !toggleStates[tweak.name];
                                        if (willOpenModal) {
                                          e.preventDefault();
                                          e.stopPropagation();
                                        }
                      ...
                      <div className="flex items-start mb-3">
                        <h2 className="font-semibold text-sparkle-text text-base">
                          {tweak.title}
                        </h2>
                      </div>
              ...
              <div className="col-span-full flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-xl font-medium mb-2 text-sparkle-text">
                  {" "}
                  Loading Tweaks...
                </h3>
